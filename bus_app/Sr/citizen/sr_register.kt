...
class sr_register : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_sr_register)

        val name = findViewById<EditText>(R.id.sr_name)
        val adhar = findViewById<EditText>(R.id.sr_card)
        val number = findViewById<EditText>(R.id.sr_monumber)
        val email = findViewById<EditText>(R.id.sr_email)
        val password = findViewById<EditText>(R.id.sr_password)
        val btn = findViewById<Button>(R.id.btn)

        auth = FirebaseAuth.getInstance()

        database = FirebaseDatabase.getInstance()
        databaseReference = database?.reference!!.child("srcitizen")
...
//                        Toast.makeText(applicationContext,adhar.text.toString(), Toast.LENGTH_LONG).show()
    ...
    private fun sharedata(value: String, email: String) {
        ...
        editor.putString("citizen-name",value)
        editor.putString("citizen-email",email)
        editor.apply()
        editor.commit()
    }
